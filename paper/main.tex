...
187 domains. The key architectural insight is that only the simulation layer is
...
\item A cross-domain analogy engine that detects 570 mathematical isomorphisms
  across 187 domains (Section~\ref{sec:cross_domain}).
...
\item Open-source implementation with 7400+ tests, publication-quality
...
across 187 domains. Brighter cells indicate stronger mathematical analogies.}
...
isomorphisms across 187 domains. The strongest analogies (strength $> 0.9$)
...
confirm that all 14 rediscovered R$^2$ values are statistically significant
...
We have verified this with 187 domains across 125 mathematical classes.
...
The total simulation code across all 187 domains is $\sim$7,000 lines,
...
570 mathematical isomorphisms across 187 domains, suggesting that scientific
...
The pipeline has been extended to 187 domains spanning 125 mathematical
classes---including
...
The full codebase, including 7400+ unit tests covering all 188 simulation
...
\begin{verbatim}
pip install -e ".[dev]"
python -m pytest tests/unit/ -q       # 7400+ tests
