...
\section{Per-Domain Discovery Details}
\label{app:domains}

Table~\ref{tab:full_results} provides the complete per-domain results
with discovered equations and simulation parameters.

\begin{table*}[h]
\centering
\caption{Full 14-domain rediscovery results with discovered equations.}
\label{tab:full_results}
\small
\begin{tabular}{llccp{5cm}}
\toprule
Domain & Math Class & Method & $R^2$ & Discovered Equation \\
\midrule
Projectile & Algebraic & \pysr{} & 1.0000 & $R = 0.1019 \cdot v_0^2 \sin(2\theta)$ \\
Lotka-Volterra & Nonlinear ODE & \sindy{} & 1.0000 & $\dot{x} = 1.1x - 0.4xy$, $\dot{y} = -0.4y + 0.1xy$ \\
Gray-Scott & PDE & \pysr{} & 0.9851 & $\lambda \sim \sqrt{D_v}$, correlation = 0.927 \\
SIR Epidemic & Nonlinear ODE & Both & 1.0000 & $R_0 = \beta/\gamma$; $\dot{S} = -\beta SI$ \\
Double Pendulum & Chaotic ODE & \pysr{} & 0.9999 & $T = \sqrt{4.03 L} \approx 2\pi\sqrt{L/g}$ \\
Harmonic Osc. & Linear ODE & Both & 1.0000 & $\omega_0 = \sqrt{k/m}$; $\dot{x} = v$, $\dot{v} = -kx/m - cv/m$ \\
Lorenz & Chaotic ODE & \sindy{} & 0.9999 & $\sigma = 9.98$, $\rho = 27.8$, $\beta = 2.66$ \\
Navier-Stokes & PDE & \pysr{} & 1.0000 & Decay rate $= 4\nu$ \\
Van der Pol & Nonlinear ODE & \pysr{} & 0.9999 & $T(\mu)$, amplitude $A = 2.01$ \\
Kuramoto & Collective ODE & \pysr{} & 0.9695 & Sync transition $r(K)$ \\
Brusselator & Nonlinear ODE & Both & 0.9964 & $b_c \approx a^2 + 0.91$ \\
FitzHugh-Nagumo & Nonlinear ODE & \sindy{} & 1.0000 & $\dot{v} = 0.5 + v - w - v^3/3$ \\
Heat Equation & Linear PDE & \pysr{} & 1.0000 & Decay rate $\lambda = D$ \\
Logistic Map & Discrete Chaos & \pysr{} & 0.6287 & $\delta \in [4.0, 4.75]$, $\lambda(r\!=\!4) = \ln 2$ \\
\bottomrule
\end{tabular}
\end{table*}
...
All 15 simulation domains (14 domains plus the Duffing template) are
verified deterministic: identical initial conditions produce identical
trajectories with maximum absolute difference of $0.0$ across all
state dimensions. The verification completes in 0.32 seconds for all
domains. Random number generators are seeded explicitly where applicable
(Kuramoto natural frequencies).

The full test suite contains 402 passing tests across 26 test files,
with 15 tests skipped (JAX-dependent world model tests on CPU-only
environments). All tests are deterministic and reproducible.
