...
class OzoneLayerSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1124*x1 - 0.0415*x0 + 0.0443*np.sin(0.0207*self._t)
        dx1 = -0.0716*x0*x1 + 0.1098*x2 - 0.0981*x1
        dx2 = 0.0434*x0 - 0.0469*x2 + 0.0774*x3 - 0.0406*x1*x2
        dx3 = 0.0821*x1*x2 - 0.0461*x3 + 0.0559*x0
