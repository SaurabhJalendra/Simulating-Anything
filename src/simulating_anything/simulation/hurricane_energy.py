...
class HurricaneEnergySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1021*x1 - 0.1017*x0 + 0.0455*np.sin(0.0640*self._t)
        dx1 = -0.0694*x0*x1 + 0.1048*x2 - 0.0642*x1
        dx2 = 0.0916*x0 - 0.1034*x2 + 0.1013*x3 - 0.0726*x1*x2
        dx3 = 0.0393*x1*x2 - 0.0363*x3 + 0.0810*x0
