...
class CircadianCortisolSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0234*x1 - 0.1024*x0 + 0.0469*np.sin(0.0651*self._t)
        dx1 = -0.0639*x0*x1 + 0.0694*x2 - 0.0410*x1
        dx2 = 0.1034*x0 - 0.0955*x2 + 0.0298*x3 - 0.0593*x1*x2
        dx3 = 0.0721*x1*x2 - 0.0561*x3 + 0.0290*x0
