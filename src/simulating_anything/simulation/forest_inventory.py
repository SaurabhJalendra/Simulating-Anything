...
class ForestInventorySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1065*x1 - 0.0334*x0 + 0.0980*np.sin(0.1092*self._t)
        dx1 = -0.1172*x0*x1 + 0.0626*x2 - 0.0622*x1
        dx2 = 0.1115*x0 - 0.0949*x2 + 0.1194*x3 - 0.0604*x1*x2
        dx3 = 0.0398*x1*x2 - 0.0565*x3 + 0.0913*x0
