...
class TermiteMoundSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0846*x1 - 0.0327*x0 + 0.0500*np.sin(0.0550*self._t)
        dx1 = -0.0494*x0*x1 + 0.1149*x2 - 0.0986*x1
        dx2 = 0.0916*x0 - 0.1187*x2 + 0.0646*x3 - 0.0777*x1*x2
        dx3 = 0.0312*x1*x2 - 0.0507*x3 + 0.0923*x0
