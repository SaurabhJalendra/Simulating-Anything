...
class BloodGlucoseInsulinSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0376*x1 - 0.0785*x0 + 0.1008*np.sin(0.0671*self._t)
        dx1 = -0.1082*x0*x1 + 0.0317*x2 - 0.0346*x1
        dx2 = 0.0507*x0 - 0.0600*x2 + 0.1218*x3 - 0.0631*x1*x2
        dx3 = 0.0605*x1*x2 - 0.1229*x3 + 0.0534*x0
