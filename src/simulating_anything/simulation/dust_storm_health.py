...
class DustStormHealthSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0935*x1 - 0.0501*x0 + 0.0281*np.sin(0.0948*self._t)
        dx1 = -0.0552*x0*x1 + 0.0256*x2 - 0.0969*x1
        dx2 = 0.0975*x0 - 0.1050*x2 + 0.0539*x3 - 0.0832*x1*x2
        dx3 = 0.0410*x1*x2 - 0.0687*x3 + 0.0680*x0
