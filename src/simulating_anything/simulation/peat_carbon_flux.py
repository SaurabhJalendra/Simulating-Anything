...
class PeatCarbonFluxSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0696*x1 - 0.1195*x0 + 0.1117*np.sin(0.0941*self._t)
        dx1 = -0.0961*x0*x1 + 0.0737*x2 - 0.1135*x1
        dx2 = 0.0867*x0 - 0.0625*x2 + 0.1174*x3 - 0.1030*x1*x2
        dx3 = 0.0762*x1*x2 - 0.0364*x3 + 0.0472*x0
