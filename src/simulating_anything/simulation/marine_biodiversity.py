...
class MarineBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1194*x1 - 0.0843*x0 + 0.1006*np.sin(0.0706*self._t)
        dx1 = -0.0966*x0*x1 + 0.0226*x2 - 0.0712*x1
        dx2 = 0.0369*x0 - 0.0266*x2 + 0.0975*x3 - 0.0723*x1*x2
        dx3 = 0.0625*x1*x2 - 0.0486*x3 + 0.0386*x0
