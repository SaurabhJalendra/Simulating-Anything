...
class BorealBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1069*x1 - 0.0473*x0 + 0.0639*np.sin(0.0868*self._t)
        dx1 = -0.1150*x0*x1 + 0.0947*x2 - 0.0414*x1
        dx2 = 0.0965*x0 - 0.0841*x2 + 0.0863*x3 - 0.0914*x1*x2
        dx3 = 0.0622*x1*x2 - 0.0955*x3 + 0.0621*x0
