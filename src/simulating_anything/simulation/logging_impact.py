...
class LoggingImpactSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0402*x1 - 0.0452*x0 + 0.0975*np.sin(0.0595*self._t)
        dx1 = -0.0415*x0*x1 + 0.0241*x2 - 0.0299*x1
        dx2 = 0.0437*x0 - 0.0384*x2 + 0.1096*x3 - 0.0604*x1*x2
        dx3 = 0.0381*x1*x2 - 0.0301*x3 + 0.0472*x0
