...
class AdrenalGlandSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0444*x1 - 0.0983*x0 + 0.1057*np.sin(0.0955*self._t)
        dx1 = -0.0600*x0*x1 + 0.0233*x2 - 0.0298*x1
        dx2 = 0.0217*x0 - 0.0804*x2 + 0.1127*x3 - 0.0283*x1*x2
        dx3 = 0.0399*x1*x2 - 0.0554*x3 + 0.0817*x0
