...
class CanopyPhotosynthesisSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0260*x1 - 0.1153*x0 + 0.0622*np.sin(0.0412*self._t)
        dx1 = -0.0226*x0*x1 + 0.0532*x2 - 0.0463*x1
        dx2 = 0.0588*x0 - 0.0949*x2 + 0.0657*x3 - 0.0471*x1*x2
        dx3 = 0.0418*x1*x2 - 0.0908*x3 + 0.1103*x0
