...
class EstuaryBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0577*x1 - 0.0514*x0 + 0.0385*np.sin(0.1115*self._t)
        dx1 = -0.1194*x0*x1 + 0.0686*x2 - 0.0334*x1
        dx2 = 0.1078*x0 - 0.1020*x2 + 0.0746*x3 - 0.0742*x1*x2
        dx3 = 0.0463*x1*x2 - 0.0443*x3 + 0.0385*x0
