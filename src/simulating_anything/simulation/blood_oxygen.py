...
class BloodOxygenSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0658*x1 - 0.0860*x0 + 0.0868*np.sin(0.0594*self._t)
        dx1 = -0.1172*x0*x1 + 0.0371*x2 - 0.0947*x1
        dx2 = 0.0734*x0 - 0.0868*x2 + 0.0556*x3 - 0.0444*x1*x2
        dx3 = 0.0404*x1*x2 - 0.0721*x3 + 0.1143*x0
