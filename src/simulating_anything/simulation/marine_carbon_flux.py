...
class MarineCarbonFluxSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0208*x1 - 0.1055*x0 + 0.0418*np.sin(0.1075*self._t)
        dx1 = -0.0763*x0*x1 + 0.1003*x2 - 0.0280*x1
        dx2 = 0.1167*x0 - 0.0380*x2 + 0.1039*x3 - 0.0678*x1*x2
        dx3 = 0.0402*x1*x2 - 0.0826*x3 + 0.0400*x0
