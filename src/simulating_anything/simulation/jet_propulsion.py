...
class JetPropulsionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0870*x1 - 0.0254*x0 + 0.0271*np.sin(0.1006*self._t)
        dx1 = -0.0668*x0*x1 + 0.0889*x2 - 0.0355*x1
        dx2 = 0.0652*x0 - 0.0253*x2 + 0.0522*x3 - 0.0774*x1*x2
        dx3 = 0.1115*x1*x2 - 0.0633*x3 + 0.0531*x0
