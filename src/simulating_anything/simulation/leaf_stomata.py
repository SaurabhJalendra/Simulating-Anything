...
class LeafStomataSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0340*x1 - 0.0422*x0 + 0.0770*np.sin(0.1006*self._t)
        dx1 = -0.0826*x0*x1 + 0.0842*x2 - 0.0511*x1
        dx2 = 0.0851*x0 - 0.0339*x2 + 0.0778*x3 - 0.0207*x1*x2
        dx3 = 0.0589*x1*x2 - 0.0775*x3 + 0.1163*x0
