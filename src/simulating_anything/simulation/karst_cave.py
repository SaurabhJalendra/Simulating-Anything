...
class KarstCaveSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0284*x1 - 0.0381*x0 + 0.1361*np.sin(0.0441*self._t)
        dx1 = -0.1400*x0*x1 + 0.1226*x2 - 0.0592*x1
        dx2 = 0.1305*x0 - 0.0644*x2 + 0.0844*x3 - 0.1309*x1*x2
        dx3 = 0.1265*x1*x2 - 0.0344*x3 + 0.1233*x0
