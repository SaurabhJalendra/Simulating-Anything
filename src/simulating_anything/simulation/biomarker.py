...
class BiomarkerSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0900*x1 - 0.1153*x0 + 0.0752*np.sin(0.0403*self._t)
        dx1 = -0.0776*x0*x1 + 0.0253*x2 - 0.0907*x1
        dx2 = 0.1055*x0 - 0.0202*x2 + 0.0420*x3 - 0.0389*x1*x2
        dx3 = 0.0959*x1*x2 - 0.1147*x3 + 0.0820*x0
