...
class GlacierBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1088*x1 - 0.0989*x0 + 0.0821*np.sin(0.0580*self._t)
        dx1 = -0.0316*x0*x1 + 0.0947*x2 - 0.0338*x1
        dx2 = 0.0280*x0 - 0.1168*x2 + 0.0490*x3 - 0.0951*x1*x2
        dx3 = 0.0611*x1*x2 - 0.0746*x3 + 0.0416*x0
