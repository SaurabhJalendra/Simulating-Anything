...
class CombustionChamberSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0494*x1 - 0.0743*x0 + 0.0821*np.sin(0.1013*self._t)
        dx1 = -0.0625*x0*x1 + 0.0742*x2 - 0.0625*x1
        dx2 = 0.0259*x0 - 0.1165*x2 + 0.0236*x3 - 0.0805*x1*x2
        dx3 = 0.0878*x1*x2 - 0.0821*x3 + 0.0341*x0
