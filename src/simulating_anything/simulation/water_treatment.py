"""Novel Coagulation + flocculation + settling simulation (4D)."""
from __future__ import annotations
import numpy as np
from simulating_anything.simulation.base import SimulationEnvironment
from simulating_anything.types.simulation import SimulationConfig

class WaterTreatmentSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.13*x1 - 0.055*x0 + 0.007*np.sin(0.06*self._t)
        dx1 = -0.085*x0*x1 + 0.028*x2 - 0.012*x1
        dx2 = 0.048*x0 - 0.032*x2 + 0.011*x3 - 0.007*x1*x2
        dx3 = 0.072*x1*x2 - 0.048*x3 + 0.006*x0
