...
class RainforestCanopySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0249*x1 - 0.0543*x0 + 0.1028*np.sin(0.1290*self._t)
        dx1 = -0.1226*x0*x1 + 0.0807*x2 - 0.1100*x1
        dx2 = 0.1428*x0 - 0.1331*x2 + 0.0585*x3 - 0.0955*x1*x2
        dx3 = 0.1652*x1*x2 - 0.1418*x3 + 0.0457*x0
