...
class TemperateForestSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0599*x1 - 0.0203*x0 + 0.0486*np.sin(0.0228*self._t)
        dx1 = -0.0781*x0*x1 + 0.0936*x2 - 0.0485*x1
        dx2 = 0.0456*x0 - 0.1090*x2 + 0.0321*x3 - 0.0522*x1*x2
        dx3 = 0.0243*x1*x2 - 0.0641*x3 + 0.0602*x0
