...
class SpinalCordSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1038*x1 - 0.0286*x0 + 0.0272*np.sin(0.0709*self._t)
        dx1 = -0.0578*x0*x1 + 0.1038*x2 - 0.0965*x1
        dx2 = 0.0791*x0 - 0.1145*x2 + 0.0578*x3 - 0.0748*x1*x2
        dx3 = 0.0690*x1*x2 - 0.1052*x3 + 0.1137*x0
