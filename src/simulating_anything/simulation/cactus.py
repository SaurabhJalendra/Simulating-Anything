...
class CactusSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0596*x1 - 0.0306*x0 + 0.0456*np.sin(0.0471*self._t)
        dx1 = -0.0677*x0*x1 + 0.0993*x2 - 0.1153*x1
        dx2 = 0.0835*x0 - 0.0505*x2 + 0.0227*x3 - 0.0517*x1*x2
        dx3 = 0.0867*x1*x2 - 0.1145*x3 + 0.0934*x0
