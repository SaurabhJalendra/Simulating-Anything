"""Novel Thermal stress + recovery + algae simulation (4D)."""
from __future__ import annotations
import numpy as np
from simulating_anything.simulation.base import SimulationEnvironment
from simulating_anything.types.simulation import SimulationConfig

class ReefBleachingRecoverySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.14*x1 - 0.05*x0 + 0.006*np.sin(0.07*self._t)
        dx1 = -0.08*x0*x1 + 0.024*x2 - 0.013*x1
        dx2 = 0.052*x0 - 0.03*x2 + 0.013*x3 - 0.008*x1*x2
        dx3 = 0.068*x1*x2 - 0.042*x3 + 0.005*x0
