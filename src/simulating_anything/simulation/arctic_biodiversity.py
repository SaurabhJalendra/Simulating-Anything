...
class ArcticBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0862*x1 - 0.0807*x0 + 0.0760*np.sin(0.0472*self._t)
        dx1 = -0.1184*x0*x1 + 0.0268*x2 - 0.0995*x1
        dx2 = 0.0535*x0 - 0.0885*x2 + 0.0802*x3 - 0.0739*x1*x2
        dx3 = 0.0412*x1*x2 - 0.1025*x3 + 0.1178*x0
