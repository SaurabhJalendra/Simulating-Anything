...
class GardenBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0639*x1 - 0.0848*x0 + 0.0768*np.sin(0.1082*self._t)
        dx1 = -0.0365*x0*x1 + 0.0575*x2 - 0.0220*x1
        dx2 = 0.1145*x0 - 0.0396*x2 + 0.1002*x3 - 0.0409*x1*x2
        dx3 = 0.0453*x1*x2 - 0.0828*x3 + 0.0719*x0
