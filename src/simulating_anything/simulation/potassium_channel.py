...
class PotassiumChannelSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0647*x1 - 0.0782*x0 + 0.0880*np.sin(0.0565*self._t)
        dx1 = -0.0549*x0*x1 + 0.0411*x2 - 0.0618*x1
        dx2 = 0.1138*x0 - 0.0829*x2 + 0.0779*x3 - 0.0406*x1*x2
        dx3 = 0.0635*x1*x2 - 0.0732*x3 + 0.0805*x0
