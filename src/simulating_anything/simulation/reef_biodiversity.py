...
class ReefBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0439*x1 - 0.0888*x0 + 0.1157*np.sin(0.0229*self._t)
        dx1 = -0.0558*x0*x1 + 0.0664*x2 - 0.0652*x1
        dx2 = 0.0887*x0 - 0.0832*x2 + 0.0863*x3 - 0.0324*x1*x2
        dx3 = 0.0575*x1*x2 - 0.0399*x3 + 0.1181*x0
