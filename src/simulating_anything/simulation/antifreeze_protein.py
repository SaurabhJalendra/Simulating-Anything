...
class AntifreezeProteinSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0515*x1 - 0.1159*x0 + 0.1100*np.sin(0.0983*self._t)
        dx1 = -0.0332*x0*x1 + 0.0443*x2 - 0.0386*x1
        dx2 = 0.0351*x0 - 0.0353*x2 + 0.0396*x3 - 0.0229*x1*x2
        dx3 = 0.0900*x1*x2 - 0.0474*x3 + 0.1100*x0
