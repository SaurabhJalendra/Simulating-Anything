...
class BreastCancerSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0496*x1 - 0.0769*x0 + 0.0253*np.sin(0.0443*self._t)
        dx1 = -0.0534*x0*x1 + 0.0714*x2 - 0.1194*x1
        dx2 = 0.0720*x0 - 0.0248*x2 + 0.0258*x3 - 0.0418*x1*x2
        dx3 = 0.0599*x1*x2 - 0.0311*x3 + 0.0737*x0
