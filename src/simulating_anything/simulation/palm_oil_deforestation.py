...
class PalmOilDeforestationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0652*x1 - 0.0753*x0 + 0.0826*np.sin(0.0716*self._t)
        dx1 = -0.0226*x0*x1 + 0.0874*x2 - 0.0553*x1
        dx2 = 0.0253*x0 - 0.0600*x2 + 0.0900*x3 - 0.0473*x1*x2
        dx3 = 0.0838*x1*x2 - 0.0243*x3 + 0.1169*x0
