...
class ParkingLotRunoffSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0934*x1 - 0.0503*x0 + 0.0827*np.sin(0.0795*self._t)
        dx1 = -0.0252*x0*x1 + 0.0320*x2 - 0.0560*x1
        dx2 = 0.0829*x0 - 0.0674*x2 + 0.0883*x3 - 0.1153*x1*x2
        dx3 = 0.0473*x1*x2 - 0.0216*x3 + 0.0287*x0
