...
class ExoplanetAtmosphereSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0600*x1 - 0.0470*x0 + 0.1171*np.sin(0.0779*self._t)
        dx1 = -0.0907*x0*x1 + 0.0806*x2 - 0.0596*x1
        dx2 = 0.0348*x0 - 0.0366*x2 + 0.1042*x3 - 0.1024*x1*x2
        dx3 = 0.1048*x1*x2 - 0.0421*x3 + 0.0612*x0
