...
class PondBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0324*x1 - 0.0778*x0 + 0.0579*np.sin(0.0269*self._t)
        dx1 = -0.0741*x0*x1 + 0.0876*x2 - 0.0497*x1
        dx2 = 0.1011*x0 - 0.1153*x2 + 0.0310*x3 - 0.0657*x1*x2
        dx3 = 0.0485*x1*x2 - 0.0874*x3 + 0.0499*x0
