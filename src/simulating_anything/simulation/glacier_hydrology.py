...
class GlacierHydrologySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1092*x1 - 0.0358*x0 + 0.0861*np.sin(0.0213*self._t)
        dx1 = -0.0440*x0*x1 + 0.1127*x2 - 0.0820*x1
        dx2 = 0.0274*x0 - 0.0319*x2 + 0.0592*x3 - 0.0703*x1*x2
        dx3 = 0.0554*x1*x2 - 0.0762*x3 + 0.0697*x0
