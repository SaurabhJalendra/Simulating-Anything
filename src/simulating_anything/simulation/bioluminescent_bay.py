...
class BioluminescentBaySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1105*x1 - 0.0361*x0 + 0.0208*np.sin(0.1159*self._t)
        dx1 = -0.0595*x0*x1 + 0.1118*x2 - 0.1090*x1
        dx2 = 0.0647*x0 - 0.0498*x2 + 0.0677*x3 - 0.0883*x1*x2
        dx3 = 0.0326*x1*x2 - 0.0209*x3 + 0.0349*x0
