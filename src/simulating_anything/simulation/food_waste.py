...
class FoodWasteSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0781*x1 - 0.0913*x0 + 0.1023*np.sin(0.0638*self._t)
        dx1 = -0.0308*x0*x1 + 0.0532*x2 - 0.0992*x1
        dx2 = 0.1105*x0 - 0.0764*x2 + 0.0585*x3 - 0.0418*x1*x2
        dx3 = 0.0454*x1*x2 - 0.0857*x3 + 0.0741*x0
