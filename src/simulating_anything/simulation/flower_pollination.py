...
class FlowerPollinationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1054*x1 - 0.1303*x0 + 0.0675*np.sin(0.0895*self._t)
        dx1 = -0.1183*x0*x1 + 0.0402*x2 - 0.0281*x1
        dx2 = 0.1138*x0 - 0.0852*x2 + 0.1663*x3 - 0.1567*x1*x2
        dx3 = 0.1630*x1*x2 - 0.0773*x3 + 0.0878*x0
