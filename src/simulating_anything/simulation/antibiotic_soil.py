...
class AntibioticSoilSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1014*x1 - 0.0731*x0 + 0.0973*np.sin(0.0442*self._t)
        dx1 = -0.0418*x0*x1 + 0.0490*x2 - 0.0575*x1
        dx2 = 0.0705*x0 - 0.0699*x2 + 0.0842*x3 - 0.1187*x1*x2
        dx3 = 0.0834*x1*x2 - 0.0742*x3 + 0.0312*x0
