...
class MiningReclamationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0642*x1 - 0.0285*x0 + 0.1130*np.sin(0.0491*self._t)
        dx1 = -0.0560*x0*x1 + 0.0992*x2 - 0.0777*x1
        dx2 = 0.1058*x0 - 0.0763*x2 + 0.0844*x3 - 0.0310*x1*x2
        dx3 = 0.0807*x1*x2 - 0.0520*x3 + 0.1004*x0
