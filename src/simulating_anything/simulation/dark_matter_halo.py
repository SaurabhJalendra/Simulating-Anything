...
class DarkMatterHaloSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0639*x1 - 0.0955*x0 + 0.0820*np.sin(0.0368*self._t)
        dx1 = -0.0460*x0*x1 + 0.0448*x2 - 0.1088*x1
        dx2 = 0.1195*x0 - 0.0844*x2 + 0.0739*x3 - 0.0328*x1*x2
        dx3 = 0.0413*x1*x2 - 0.0450*x3 + 0.0788*x0
