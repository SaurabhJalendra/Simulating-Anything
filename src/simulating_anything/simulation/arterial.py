...
class ArterialSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0899*x1 - 0.0769*x0 + 0.0933*np.sin(0.1167*self._t)
        dx1 = -0.0803*x0*x1 + 0.0549*x2 - 0.0884*x1
        dx2 = 0.1186*x0 - 0.0566*x2 + 0.0270*x3 - 0.0812*x1*x2
        dx3 = 0.0811*x1*x2 - 0.1103*x3 + 0.0752*x0
