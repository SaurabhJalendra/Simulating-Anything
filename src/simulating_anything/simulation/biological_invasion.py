...
class BiologicalInvasionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1180*x1 - 0.0382*x0 + 0.0571*np.sin(0.0925*self._t)
        dx1 = -0.0835*x0*x1 + 0.0441*x2 - 0.0859*x1
        dx2 = 0.0308*x0 - 0.0299*x2 + 0.0679*x3 - 0.0343*x1*x2
        dx3 = 0.0348*x1*x2 - 0.0636*x3 + 0.1149*x0
