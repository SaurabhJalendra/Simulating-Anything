...
class MarshBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0377*x1 - 0.0600*x0 + 0.0934*np.sin(0.0310*self._t)
        dx1 = -0.0974*x0*x1 + 0.0358*x2 - 0.1121*x1
        dx2 = 0.0507*x0 - 0.0535*x2 + 0.0460*x3 - 0.0641*x1*x2
        dx3 = 0.1196*x1*x2 - 0.0611*x3 + 0.1141*x0
