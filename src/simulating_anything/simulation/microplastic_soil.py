...
class MicroplasticSoilSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0684*x1 - 0.0215*x0 + 0.1043*np.sin(0.0241*self._t)
        dx1 = -0.0721*x0*x1 + 0.0472*x2 - 0.0433*x1
        dx2 = 0.0335*x0 - 0.0528*x2 + 0.0401*x3 - 0.1152*x1*x2
        dx3 = 0.0666*x1*x2 - 0.0327*x3 + 0.0477*x0
