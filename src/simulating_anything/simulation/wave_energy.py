...
class WaveEnergySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1193*x1 - 0.0744*x0 + 0.0501*np.sin(0.1105*self._t)
        dx1 = -0.1055*x0*x1 + 0.0329*x2 - 0.0269*x1
        dx2 = 0.0475*x0 - 0.1052*x2 + 0.1011*x3 - 0.0215*x1*x2
        dx3 = 0.0835*x1*x2 - 0.0748*x3 + 0.1154*x0
