...
class PeatCarbonSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0993*x1 - 0.0611*x0 + 0.0299*np.sin(0.1126*self._t)
        dx1 = -0.0988*x0*x1 + 0.0980*x2 - 0.1112*x1
        dx2 = 0.0638*x0 - 0.0293*x2 + 0.0765*x3 - 0.0534*x1*x2
        dx3 = 0.0738*x1*x2 - 0.0337*x3 + 0.0642*x0
