...
class DolphinEcholocationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0930*x1 - 0.0890*x0 + 0.0728*np.sin(0.0420*self._t)
        dx1 = -0.0309*x0*x1 + 0.0333*x2 - 0.0382*x1
        dx2 = 0.0245*x0 - 0.0963*x2 + 0.1022*x3 - 0.0611*x1*x2
        dx3 = 0.0215*x1*x2 - 0.0488*x3 + 0.0698*x0
