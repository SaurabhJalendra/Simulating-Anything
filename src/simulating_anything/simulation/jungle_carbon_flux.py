...
class JungleCarbonFluxSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0517*x1 - 0.1115*x0 + 0.0945*np.sin(0.0786*self._t)
        dx1 = -0.0457*x0*x1 + 0.0632*x2 - 0.0376*x1
        dx2 = 0.1021*x0 - 0.0729*x2 + 0.0318*x3 - 0.0769*x1*x2
        dx3 = 0.0828*x1*x2 - 0.0901*x3 + 0.0776*x0
