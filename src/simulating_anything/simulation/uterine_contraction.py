...
class UterineContractionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0703*x1 - 0.1111*x0 + 0.1093*np.sin(0.0404*self._t)
        dx1 = -0.0617*x0*x1 + 0.0884*x2 - 0.0430*x1
        dx2 = 0.0404*x0 - 0.0560*x2 + 0.1166*x3 - 0.0784*x1*x2
        dx3 = 0.0450*x1*x2 - 0.0494*x3 + 0.0244*x0
