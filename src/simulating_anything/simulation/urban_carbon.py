...
class UrbanCarbonSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0856*x1 - 0.1041*x0 + 0.0552*np.sin(0.1172*self._t)
        dx1 = -0.0360*x0*x1 + 0.0743*x2 - 0.0777*x1
        dx2 = 0.0291*x0 - 0.1115*x2 + 0.1149*x3 - 0.0297*x1*x2
        dx3 = 0.0806*x1*x2 - 0.1041*x3 + 0.0467*x0
