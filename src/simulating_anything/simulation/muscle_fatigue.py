...
class MuscleFatigueSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0398*x1 - 0.0508*x0 + 0.0871*np.sin(0.0443*self._t)
        dx1 = -0.1133*x0*x1 + 0.0921*x2 - 0.0963*x1
        dx2 = 0.0249*x0 - 0.0253*x2 + 0.0561*x3 - 0.1058*x1*x2
        dx3 = 0.0285*x1*x2 - 0.0713*x3 + 0.0813*x0
