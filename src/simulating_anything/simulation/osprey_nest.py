...
class OspreyNestSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0713*x1 - 0.0410*x0 + 0.1131*np.sin(0.0486*self._t)
        dx1 = -0.0803*x0*x1 + 0.0550*x2 - 0.1174*x1
        dx2 = 0.0382*x0 - 0.0234*x2 + 0.0370*x3 - 0.0563*x1*x2
        dx3 = 0.1144*x1*x2 - 0.1090*x3 + 0.0699*x0
