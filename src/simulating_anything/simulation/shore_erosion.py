...
class ShoreErosionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0356*x1 - 0.0325*x0 + 0.0569*np.sin(0.1192*self._t)
        dx1 = -0.1176*x0*x1 + 0.1094*x2 - 0.0413*x1
        dx2 = 0.0683*x0 - 0.0915*x2 + 0.1118*x3 - 0.0644*x1*x2
        dx3 = 0.0553*x1*x2 - 0.1041*x3 + 0.1067*x0
