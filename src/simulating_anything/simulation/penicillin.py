...
class PenicillinSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1199*x1 - 0.0353*x0 + 0.0328*np.sin(0.0825*self._t)
        dx1 = -0.0299*x0*x1 + 0.0919*x2 - 0.0690*x1
        dx2 = 0.1158*x0 - 0.0559*x2 + 0.0475*x3 - 0.0978*x1*x2
        dx3 = 0.1055*x1*x2 - 0.0982*x3 + 0.0566*x0
