...
class PollinatorDeclineSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1061*x1 - 0.0724*x0 + 0.0811*np.sin(0.0552*self._t)
        dx1 = -0.0454*x0*x1 + 0.0589*x2 - 0.0370*x1
        dx2 = 0.0937*x0 - 0.0346*x2 + 0.0801*x3 - 0.0649*x1*x2
        dx3 = 0.0506*x1*x2 - 0.0384*x3 + 0.0698*x0
