...
class ZincDeficiencySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0590*x1 - 0.0738*x0 + 0.1185*np.sin(0.0840*self._t)
        dx1 = -0.0809*x0*x1 + 0.0790*x2 - 0.0993*x1
        dx2 = 0.0248*x0 - 0.0652*x2 + 0.0234*x3 - 0.0753*x1*x2
        dx3 = 0.0747*x1*x2 - 0.0279*x3 + 0.0510*x0
