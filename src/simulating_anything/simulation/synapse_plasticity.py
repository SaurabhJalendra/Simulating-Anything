...
class SynapsePlasticitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0682*x1 - 0.0998*x0 + 0.0305*np.sin(0.1085*self._t)
        dx1 = -0.0648*x0*x1 + 0.0900*x2 - 0.0643*x1
        dx2 = 0.0553*x0 - 0.0633*x2 + 0.0584*x3 - 0.1160*x1*x2
        dx3 = 0.0496*x1*x2 - 0.0627*x3 + 0.1065*x0
