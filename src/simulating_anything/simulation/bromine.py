...
class BromineSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1160*x1 - 0.0245*x0 + 0.0359*np.sin(0.0975*self._t)
        dx1 = -0.1033*x0*x1 + 0.0344*x2 - 0.1180*x1
        dx2 = 0.0902*x0 - 0.0654*x2 + 0.0552*x3 - 0.0887*x1*x2
        dx3 = 0.0276*x1*x2 - 0.1125*x3 + 0.1003*x0
