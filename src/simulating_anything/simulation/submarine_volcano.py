...
class SubmarineVolcanoSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0267*x1 - 0.0457*x0 + 0.0769*np.sin(0.0732*self._t)
        dx1 = -0.0712*x0*x1 + 0.0357*x2 - 0.0228*x1
        dx2 = 0.1180*x0 - 0.0565*x2 + 0.0222*x3 - 0.0811*x1*x2
        dx3 = 0.0956*x1*x2 - 0.0878*x3 + 0.1120*x0
