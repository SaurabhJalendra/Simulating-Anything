...
class BasinSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1174*x1 - 0.0993*x0 + 0.0370*np.sin(0.1138*self._t)
        dx1 = -0.0696*x0*x1 + 0.0779*x2 - 0.0411*x1
        dx2 = 0.0998*x0 - 0.1078*x2 + 0.0976*x3 - 0.0857*x1*x2
        dx3 = 0.0929*x1*x2 - 0.0568*x3 + 0.0421*x0
