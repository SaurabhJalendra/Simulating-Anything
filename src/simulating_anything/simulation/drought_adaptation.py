...
class DroughtAdaptationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0472*x1 - 0.0239*x0 + 0.0290*np.sin(0.0797*self._t)
        dx1 = -0.1044*x0*x1 + 0.0233*x2 - 0.0520*x1
        dx2 = 0.0400*x0 - 0.0381*x2 + 0.0212*x3 - 0.0953*x1*x2
        dx3 = 0.0512*x1*x2 - 0.0935*x3 + 0.0269*x0
