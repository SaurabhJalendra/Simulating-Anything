...
class OrganTransplantSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0772*x1 - 0.0980*x0 + 0.0920*np.sin(0.0677*self._t)
        dx1 = -0.0792*x0*x1 + 0.1081*x2 - 0.0823*x1
        dx2 = 0.0302*x0 - 0.0360*x2 + 0.1075*x3 - 0.0314*x1*x2
        dx3 = 0.1007*x1*x2 - 0.1088*x3 + 0.1080*x0
