...
class CombustionEngineSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0300*x1 - 0.0505*x0 + 0.1057*np.sin(0.0552*self._t)
        dx1 = -0.0513*x0*x1 + 0.0956*x2 - 0.0867*x1
        dx2 = 0.0684*x0 - 0.0496*x2 + 0.0846*x3 - 0.0833*x1*x2
        dx3 = 0.0495*x1*x2 - 0.0690*x3 + 0.1108*x0
