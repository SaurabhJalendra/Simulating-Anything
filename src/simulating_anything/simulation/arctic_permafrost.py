...
class ArcticPermafrostSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0533*x1 - 0.0567*x0 + 0.0770*np.sin(0.1098*self._t)
        dx1 = -0.0943*x0*x1 + 0.0560*x2 - 0.0637*x1
        dx2 = 0.1009*x0 - 0.0233*x2 + 0.0302*x3 - 0.0742*x1*x2
        dx3 = 0.0749*x1*x2 - 0.1186*x3 + 0.0327*x0
