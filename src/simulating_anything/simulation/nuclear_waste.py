...
class NuclearWasteSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0280*x1 - 0.0813*x0 + 0.0387*np.sin(0.0895*self._t)
        dx1 = -0.0930*x0*x1 + 0.1071*x2 - 0.0730*x1
        dx2 = 0.0517*x0 - 0.1115*x2 + 0.0893*x3 - 0.0891*x1*x2
        dx3 = 0.0993*x1*x2 - 0.1024*x3 + 0.0338*x0
