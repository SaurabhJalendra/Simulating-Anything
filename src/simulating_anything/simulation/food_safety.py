...
class FoodSafetySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0514*x1 - 0.0684*x0 + 0.0927*np.sin(0.1166*self._t)
        dx1 = -0.0328*x0*x1 + 0.0804*x2 - 0.0962*x1
        dx2 = 0.0593*x0 - 0.0774*x2 + 0.0857*x3 - 0.0220*x1*x2
        dx3 = 0.0911*x1*x2 - 0.1032*x3 + 0.0370*x0
