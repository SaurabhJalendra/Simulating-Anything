...
class AerosolClimateSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0641*x1 - 0.0860*x0 + 0.1064*np.sin(0.0474*self._t)
        dx1 = -0.0935*x0*x1 + 0.1150*x2 - 0.0683*x1
        dx2 = 0.0571*x0 - 0.0622*x2 + 0.0642*x3 - 0.0437*x1*x2
        dx3 = 0.0622*x1*x2 - 0.0700*x3 + 0.0344*x0
