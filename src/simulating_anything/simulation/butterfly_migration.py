...
class ButterflyMigrationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0717*x1 - 0.1019*x0 + 0.0546*np.sin(0.0901*self._t)
        dx1 = -0.0743*x0*x1 + 0.0336*x2 - 0.0479*x1
        dx2 = 0.1195*x0 - 0.0380*x2 + 0.0248*x3 - 0.0740*x1*x2
        dx3 = 0.0719*x1*x2 - 0.0218*x3 + 0.0330*x0
