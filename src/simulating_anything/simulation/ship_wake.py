...
class ShipWakeSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0782*x1 - 0.0640*x0 + 0.1166*np.sin(0.0360*self._t)
        dx1 = -0.0616*x0*x1 + 0.1140*x2 - 0.1012*x1
        dx2 = 0.0961*x0 - 0.1118*x2 + 0.0773*x3 - 0.1102*x1*x2
        dx3 = 0.0460*x1*x2 - 0.0258*x3 + 0.0205*x0
