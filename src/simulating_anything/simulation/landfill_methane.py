"""Novel Waste decomposition + methane generation simulation (4D)."""
from __future__ import annotations
import numpy as np
from simulating_anything.simulation.base import SimulationEnvironment
from simulating_anything.types.simulation import SimulationConfig

class LandfillMethaneSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.12*x1 - 0.04*x0 + 0.01*np.sin(0.1*self._t)
        dx1 = -0.07*x0*x1 + 0.03*x2 - 0.02*x1
        dx2 = 0.04*x0 - 0.025*x2 + 0.015*x3 - 0.01*x1*x2
        dx3 = 0.08*x1*x2 - 0.06*x3 + 0.005*x0
        return np.array([dx0, dx1, dx2, dx3])
    def reset(self, seed=None):
        if seed is not None:
            rng = np.random.default_rng(seed)
            self._state = np.clip(np.array([1+rng.normal(0,.1), .5+rng.normal(0,.05),
                                            .1+rng.normal(0,.01), rng.normal(0,.01)], dtype=np.float64), 0, None)
        else:
            self._state = np.array([1.0, 0.5, 0.1, 0.0], dtype=np.float64)
        self._t = 0.0
        return self.observe()
