...
class WaterCycleSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1103*x1 - 0.1005*x0 + 0.0929*np.sin(0.1133*self._t)
        dx1 = -0.0938*x0*x1 + 0.0413*x2 - 0.0773*x1
        dx2 = 0.1089*x0 - 0.0448*x2 + 0.0727*x3 - 0.0993*x1*x2
        dx3 = 0.0785*x1*x2 - 0.0259*x3 + 0.0834*x0
