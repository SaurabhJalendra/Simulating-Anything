...
class MercuryFishSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0836*x1 - 0.0896*x0 + 0.0583*np.sin(0.1084*self._t)
        dx1 = -0.0462*x0*x1 + 0.1181*x2 - 0.1031*x1
        dx2 = 0.0304*x0 - 0.0505*x2 + 0.0250*x3 - 0.0338*x1*x2
        dx3 = 0.0797*x1*x2 - 0.0882*x3 + 0.0700*x0
