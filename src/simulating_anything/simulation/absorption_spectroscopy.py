...
class AbsorptionSpectroscopySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0751*x1 - 0.0261*x0 + 0.0371*np.sin(0.0286*self._t)
        dx1 = -0.1098*x0*x1 + 0.0643*x2 - 0.0972*x1
        dx2 = 0.1135*x0 - 0.0313*x2 + 0.0789*x3 - 0.0705*x1*x2
        dx3 = 0.0233*x1*x2 - 0.0260*x3 + 0.0338*x0
