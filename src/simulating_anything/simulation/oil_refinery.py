...
class OilRefinerySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0246*x1 - 0.0571*x0 + 0.0584*np.sin(0.0578*self._t)
        dx1 = -0.1045*x0*x1 + 0.0258*x2 - 0.0541*x1
        dx2 = 0.0526*x0 - 0.0618*x2 + 0.0427*x3 - 0.0629*x1*x2
        dx3 = 0.0416*x1*x2 - 0.1180*x3 + 0.0498*x0
