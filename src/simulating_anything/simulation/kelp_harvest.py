...
class KelpHarvestSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0794*x1 - 0.0686*x0 + 0.1328*np.sin(0.1136*self._t)
        dx1 = -0.0932*x0*x1 + 0.0704*x2 - 0.0317*x1
        dx2 = 0.0538*x0 - 0.0627*x2 + 0.0981*x3 - 0.0932*x1*x2
        dx3 = 0.0249*x1*x2 - 0.1012*x3 + 0.0934*x0
