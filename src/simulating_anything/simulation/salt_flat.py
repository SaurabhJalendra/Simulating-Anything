...
class SaltFlatSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0333*x1 - 0.0877*x0 + 0.1656*np.sin(0.1340*self._t)
        dx1 = -0.1607*x0*x1 + 0.1306*x2 - 0.1503*x1
        dx2 = 0.1030*x0 - 0.0218*x2 + 0.0810*x3 - 0.0514*x1*x2
        dx3 = 0.0770*x1*x2 - 0.1646*x3 + 0.0412*x0
