"""Novel Reverse osmosis + brine + energy simulation (4D)."""
from __future__ import annotations
import numpy as np
from simulating_anything.simulation.base import SimulationEnvironment
from simulating_anything.types.simulation import SimulationConfig

class DesalinationPlantSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.15*x1 - 0.06*x0 + 0.008*np.sin(0.05*self._t)
        dx1 = -0.09*x0*x1 + 0.025*x2 - 0.015*x1
        dx2 = 0.06*x0 - 0.035*x2 + 0.012*x3 - 0.008*x1*x2
        dx3 = 0.07*x1*x2 - 0.04*x3 + 0.003*x0
