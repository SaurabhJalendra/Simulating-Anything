...
class ChromosomeSegregationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0255*x1 - 0.0787*x0 + 0.0529*np.sin(0.0325*self._t)
        dx1 = -0.0360*x0*x1 + 0.1013*x2 - 0.0921*x1
        dx2 = 0.0472*x0 - 0.0760*x2 + 0.0969*x3 - 0.0446*x1*x2
        dx3 = 0.1168*x1*x2 - 0.0857*x3 + 0.0882*x0
