...
class PolarBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0921*x1 - 0.1035*x0 + 0.0869*np.sin(0.0654*self._t)
        dx1 = -0.0672*x0*x1 + 0.0504*x2 - 0.0935*x1
        dx2 = 0.0647*x0 - 0.1002*x2 + 0.0700*x3 - 0.1180*x1*x2
        dx3 = 0.1099*x1*x2 - 0.0525*x3 + 0.0779*x0
