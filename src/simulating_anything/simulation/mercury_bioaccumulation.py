...
class MercuryBioaccumulationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1118*x1 - 0.0805*x0 + 0.0565*np.sin(0.0450*self._t)
        dx1 = -0.0337*x0*x1 + 0.1116*x2 - 0.1058*x1
        dx2 = 0.0874*x0 - 0.0705*x2 + 0.1154*x3 - 0.0861*x1*x2
        dx3 = 0.1450*x1*x2 - 0.0346*x3 + 0.0854*x0
