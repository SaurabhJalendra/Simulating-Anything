...
class ElectromagneticWaveSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0728*x1 - 0.1016*x0 + 0.0973*np.sin(0.0722*self._t)
        dx1 = -0.1021*x0*x1 + 0.0212*x2 - 0.0986*x1
        dx2 = 0.0837*x0 - 0.0928*x2 + 0.0608*x3 - 0.1143*x1*x2
        dx3 = 0.0317*x1*x2 - 0.0261*x3 + 0.0510*x0
