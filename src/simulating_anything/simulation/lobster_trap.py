...
class LobsterTrapSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0422*x1 - 0.1048*x0 + 0.1198*np.sin(0.0259*self._t)
        dx1 = -0.1093*x0*x1 + 0.0279*x2 - 0.0724*x1
        dx2 = 0.0600*x0 - 0.1180*x2 + 0.0991*x3 - 0.0716*x1*x2
        dx3 = 0.0326*x1*x2 - 0.0628*x3 + 0.1079*x0
