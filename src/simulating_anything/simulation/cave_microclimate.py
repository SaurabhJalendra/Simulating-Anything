...
class CaveMicroclimateSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1284*x1 - 0.0656*x0 + 0.0487*np.sin(0.0433*self._t)
        dx1 = -0.0391*x0*x1 + 0.0707*x2 - 0.1259*x1
        dx2 = 0.1001*x0 - 0.0864*x2 + 0.0315*x3 - 0.1135*x1*x2
        dx3 = 0.0490*x1*x2 - 0.0472*x3 + 0.1311*x0
