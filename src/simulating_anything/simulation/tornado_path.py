...
class TornadoPathSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0404*x1 - 0.1088*x0 + 0.0245*np.sin(0.0687*self._t)
        dx1 = -0.0789*x0*x1 + 0.1157*x2 - 0.0445*x1
        dx2 = 0.0245*x0 - 0.1178*x2 + 0.1136*x3 - 0.0942*x1*x2
        dx3 = 0.0842*x1*x2 - 0.0330*x3 + 0.0997*x0
