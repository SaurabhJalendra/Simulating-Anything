...
class AquiferDepletionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0788*x1 - 0.0700*x0 + 0.0378*np.sin(0.0387*self._t)
        dx1 = -0.0825*x0*x1 + 0.1029*x2 - 0.0305*x1
        dx2 = 0.0268*x0 - 0.0659*x2 + 0.0902*x3 - 0.1143*x1*x2
        dx3 = 0.0949*x1*x2 - 0.0575*x3 + 0.0630*x0
