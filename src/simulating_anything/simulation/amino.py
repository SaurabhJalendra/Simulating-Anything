...
class AminoSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0940*x1 - 0.0535*x0 + 0.0518*np.sin(0.0560*self._t)
        dx1 = -0.0390*x0*x1 + 0.1048*x2 - 0.0927*x1
        dx2 = 0.0660*x0 - 0.1046*x2 + 0.0237*x3 - 0.0229*x1*x2
        dx3 = 0.0212*x1*x2 - 0.0205*x3 + 0.0940*x0
