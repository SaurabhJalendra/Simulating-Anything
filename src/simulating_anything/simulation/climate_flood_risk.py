...
class ClimateFloodRiskSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0715*x1 - 0.0856*x0 + 0.1068*np.sin(0.0769*self._t)
        dx1 = -0.0308*x0*x1 + 0.0575*x2 - 0.1154*x1
        dx2 = 0.0396*x0 - 0.0576*x2 + 0.0463*x3 - 0.0930*x1*x2
        dx3 = 0.0632*x1*x2 - 0.0493*x3 + 0.0415*x0
