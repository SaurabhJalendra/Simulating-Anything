...
class AlpineCarbonFluxSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0896*x1 - 0.0259*x0 + 0.0872*np.sin(0.0239*self._t)
        dx1 = -0.0416*x0*x1 + 0.0220*x2 - 0.0412*x1
        dx2 = 0.1009*x0 - 0.1088*x2 + 0.1001*x3 - 0.0857*x1*x2
        dx3 = 0.1152*x1*x2 - 0.1093*x3 + 0.0647*x0
