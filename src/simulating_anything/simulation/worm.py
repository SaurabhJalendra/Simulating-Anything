...
class WormSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1021*x1 - 0.0310*x0 + 0.0733*np.sin(0.0239*self._t)
        dx1 = -0.1159*x0*x1 + 0.0244*x2 - 0.0294*x1
        dx2 = 0.0808*x0 - 0.0837*x2 + 0.1148*x3 - 0.0657*x1*x2
        dx3 = 0.0565*x1*x2 - 0.1065*x3 + 0.0205*x0
