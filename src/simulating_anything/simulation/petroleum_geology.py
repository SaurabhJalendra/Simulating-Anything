...
class PetroleumGeologySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0929*x1 - 0.0694*x0 + 0.0813*np.sin(0.0355*self._t)
        dx1 = -0.0760*x0*x1 + 0.0591*x2 - 0.0495*x1
        dx2 = 0.0633*x0 - 0.0918*x2 + 0.0250*x3 - 0.1001*x1*x2
        dx3 = 0.1152*x1*x2 - 0.0637*x3 + 0.0339*x0
