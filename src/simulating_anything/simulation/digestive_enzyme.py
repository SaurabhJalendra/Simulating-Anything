...
class DigestiveEnzymeSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0941*x1 - 0.0892*x0 + 0.1149*np.sin(0.1104*self._t)
        dx1 = -0.1047*x0*x1 + 0.0780*x2 - 0.1155*x1
        dx2 = 0.1064*x0 - 0.0284*x2 + 0.0776*x3 - 0.0267*x1*x2
        dx3 = 0.0706*x1*x2 - 0.0385*x3 + 0.1100*x0
