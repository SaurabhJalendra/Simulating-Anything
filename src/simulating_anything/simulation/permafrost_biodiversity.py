...
class PermafrostBiodiversitySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0839*x1 - 0.1182*x0 + 0.0398*np.sin(0.1174*self._t)
        dx1 = -0.0202*x0*x1 + 0.0383*x2 - 0.0882*x1
        dx2 = 0.0801*x0 - 0.0975*x2 + 0.1030*x3 - 0.0380*x1*x2
        dx3 = 0.0995*x1*x2 - 0.0505*x3 + 0.0919*x0
