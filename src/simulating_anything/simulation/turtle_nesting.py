...
class TurtleNestingSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0409*x1 - 0.0773*x0 + 0.0805*np.sin(0.0326*self._t)
        dx1 = -0.0606*x0*x1 + 0.0513*x2 - 0.0436*x1
        dx2 = 0.0466*x0 - 0.0695*x2 + 0.0831*x3 - 0.0700*x1*x2
        dx3 = 0.0297*x1*x2 - 0.0303*x3 + 0.0344*x0
