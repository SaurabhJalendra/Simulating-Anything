...
class AnchovyFisherySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1107*x1 - 0.0970*x0 + 0.1124*np.sin(0.0645*self._t)
        dx1 = -0.0290*x0*x1 + 0.0972*x2 - 0.0309*x1
        dx2 = 0.0706*x0 - 0.1015*x2 + 0.0877*x3 - 0.0472*x1*x2
        dx3 = 0.0770*x1*x2 - 0.0909*x3 + 0.1053*x0
