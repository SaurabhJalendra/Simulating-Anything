...
class MarbleQuarrySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0787*x1 - 0.0659*x0 + 0.1052*np.sin(0.0336*self._t)
        dx1 = -0.0670*x0*x1 + 0.0309*x2 - 0.0940*x1
        dx2 = 0.0412*x0 - 0.0210*x2 + 0.0553*x3 - 0.1087*x1*x2
        dx3 = 0.0258*x1*x2 - 0.0610*x3 + 0.0513*x0
