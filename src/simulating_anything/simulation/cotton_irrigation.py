...
class CottonIrrigationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0727*x1 - 0.1055*x0 + 0.0341*np.sin(0.0363*self._t)
        dx1 = -0.0537*x0*x1 + 0.0478*x2 - 0.0352*x1
        dx2 = 0.0424*x0 - 0.1026*x2 + 0.0851*x3 - 0.0590*x1*x2
        dx3 = 0.0432*x1*x2 - 0.0677*x3 + 0.1193*x0
