...
class AtmosphereChemistrySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0939*x1 - 0.0705*x0 + 0.0831*np.sin(0.0961*self._t)
        dx1 = -0.0565*x0*x1 + 0.0802*x2 - 0.0343*x1
        dx2 = 0.1122*x0 - 0.0777*x2 + 0.0685*x3 - 0.0561*x1*x2
        dx3 = 0.0565*x1*x2 - 0.0813*x3 + 0.0583*x0
