...
class DnaRepairSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0277*x1 - 0.1024*x0 + 0.0842*np.sin(0.1041*self._t)
        dx1 = -0.0744*x0*x1 + 0.0521*x2 - 0.0920*x1
        dx2 = 0.0661*x0 - 0.0205*x2 + 0.1045*x3 - 0.0543*x1*x2
        dx3 = 0.0853*x1*x2 - 0.0588*x3 + 0.0405*x0
