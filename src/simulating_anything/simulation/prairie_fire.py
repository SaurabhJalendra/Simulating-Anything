...
class PrairieFireSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0642*x1 - 0.1199*x0 + 0.1219*np.sin(0.0876*self._t)
        dx1 = -0.1538*x0*x1 + 0.0352*x2 - 0.0404*x1
        dx2 = 0.0748*x0 - 0.0907*x2 + 0.0752*x3 - 0.0453*x1*x2
        dx3 = 0.1163*x1*x2 - 0.0965*x3 + 0.0396*x0
