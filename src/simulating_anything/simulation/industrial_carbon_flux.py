...
class IndustrialCarbonFluxSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0499*x1 - 0.1019*x0 + 0.1161*np.sin(0.0491*self._t)
        dx1 = -0.0271*x0*x1 + 0.0620*x2 - 0.0734*x1
        dx2 = 0.0252*x0 - 0.0792*x2 + 0.0888*x3 - 0.0906*x1*x2
        dx3 = 0.0525*x1*x2 - 0.0791*x3 + 0.0716*x0
