...
class RainAcidificationSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1088*x1 - 0.1102*x0 + 0.0899*np.sin(0.0899*self._t)
        dx1 = -0.0718*x0*x1 + 0.1142*x2 - 0.0695*x1
        dx2 = 0.0921*x0 - 0.0553*x2 + 0.1193*x3 - 0.0427*x1*x2
        dx3 = 0.0690*x1*x2 - 0.0473*x3 + 0.0245*x0
