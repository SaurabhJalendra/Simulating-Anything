...
class PeatFireSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0496*x1 - 0.0917*x0 + 0.0898*np.sin(0.0533*self._t)
        dx1 = -0.0374*x0*x1 + 0.0683*x2 - 0.0576*x1
        dx2 = 0.1085*x0 - 0.0369*x2 + 0.0522*x3 - 0.0570*x1*x2
        dx3 = 0.1115*x1*x2 - 0.0896*x3 + 0.0777*x0
