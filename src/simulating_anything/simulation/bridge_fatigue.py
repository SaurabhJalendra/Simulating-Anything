...
class BridgeFatigueSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1152*x1 - 0.0680*x0 + 0.0619*np.sin(0.0666*self._t)
        dx1 = -0.0880*x0*x1 + 0.0836*x2 - 0.0446*x1
        dx2 = 0.0365*x0 - 0.1171*x2 + 0.0665*x3 - 0.0760*x1*x2
        dx3 = 0.0610*x1*x2 - 0.0876*x3 + 0.0930*x0
