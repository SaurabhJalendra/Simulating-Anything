...
class DustEmissionSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1106*x1 - 0.1044*x0 + 0.0400*np.sin(0.0297*self._t)
        dx1 = -0.0909*x0*x1 + 0.0332*x2 - 0.0931*x1
        dx2 = 0.0873*x0 - 0.0312*x2 + 0.0755*x3 - 0.1118*x1*x2
        dx3 = 0.1107*x1*x2 - 0.0501*x3 + 0.0237*x0
