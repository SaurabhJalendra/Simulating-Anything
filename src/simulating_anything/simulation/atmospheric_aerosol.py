...
class AtmosphericAerosolSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1150*x1 - 0.0884*x0 + 0.0519*np.sin(0.0337*self._t)
        dx1 = -0.0218*x0*x1 + 0.0953*x2 - 0.0735*x1
        dx2 = 0.0640*x0 - 0.0848*x2 + 0.1114*x3 - 0.1160*x1*x2
        dx3 = 0.0689*x1*x2 - 0.1063*x3 + 0.0313*x0
