...
class GeneTherapySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.1035*x1 - 0.0987*x0 + 0.1148*np.sin(0.0521*self._t)
        dx1 = -0.0558*x0*x1 + 0.0601*x2 - 0.0798*x1
        dx2 = 0.0513*x0 - 0.1044*x2 + 0.0727*x3 - 0.0813*x1*x2
        dx3 = 0.0715*x1*x2 - 0.0989*x3 + 0.0380*x0
