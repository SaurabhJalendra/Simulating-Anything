...
class BiomeSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0264*x1 - 0.0395*x0 + 0.0368*np.sin(0.1140*self._t)
        dx1 = -0.0267*x0*x1 + 0.0854*x2 - 0.0683*x1
        dx2 = 0.0971*x0 - 0.0749*x2 + 0.0602*x3 - 0.1166*x1*x2
        dx3 = 0.0644*x1*x2 - 0.0614*x3 + 0.1114*x0
