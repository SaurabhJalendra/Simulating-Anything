...
class FractureMechanicsSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0264*x1 - 0.1008*x0 + 0.0432*np.sin(0.0538*self._t)
        dx1 = -0.0931*x0*x1 + 0.0887*x2 - 0.1017*x1
        dx2 = 0.0294*x0 - 0.0600*x2 + 0.0582*x3 - 0.0401*x1*x2
        dx3 = 0.0229*x1*x2 - 0.0609*x3 + 0.0799*x0
