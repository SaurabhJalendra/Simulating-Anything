...
class PesticideRunoffSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0581*x1 - 0.0234*x0 + 0.1073*np.sin(0.1034*self._t)
        dx1 = -0.0327*x0*x1 + 0.0815*x2 - 0.0899*x1
        dx2 = 0.0680*x0 - 0.0528*x2 + 0.0876*x3 - 0.0264*x1*x2
        dx3 = 0.0902*x1*x2 - 0.1000*x3 + 0.1076*x0
