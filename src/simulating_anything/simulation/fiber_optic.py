...
class FiberOpticSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0583*x1 - 0.1159*x0 + 0.0868*np.sin(0.0589*self._t)
        dx1 = -0.0478*x0*x1 + 0.1182*x2 - 0.0739*x1
        dx2 = 0.0416*x0 - 0.0641*x2 + 0.1169*x3 - 0.0741*x1*x2
        dx3 = 0.0633*x1*x2 - 0.0699*x3 + 0.0463*x0
