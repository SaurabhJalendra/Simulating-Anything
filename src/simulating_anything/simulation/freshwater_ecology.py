...
class FreshwaterEcologySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0801*x1 - 0.0331*x0 + 0.0496*np.sin(0.0208*self._t)
        dx1 = -0.0646*x0*x1 + 0.0737*x2 - 0.0562*x1
        dx2 = 0.0212*x0 - 0.1105*x2 + 0.1189*x3 - 0.0438*x1*x2
        dx3 = 0.1050*x1*x2 - 0.1015*x3 + 0.0792*x0
