...
class CellAgingSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0622*x1 - 0.0857*x0 + 0.0412*np.sin(0.0440*self._t)
        dx1 = -0.0383*x0*x1 + 0.1038*x2 - 0.1046*x1
        dx2 = 0.0769*x0 - 0.0620*x2 + 0.0935*x3 - 0.1159*x1*x2
        dx3 = 0.0828*x1*x2 - 0.1097*x3 + 0.0991*x0
