...
class AntColonyForagingSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0881*x1 - 0.0233*x0 + 0.0883*np.sin(0.0418*self._t)
        dx1 = -0.0279*x0*x1 + 0.1256*x2 - 0.0634*x1
        dx2 = 0.1688*x0 - 0.0827*x2 + 0.0906*x3 - 0.0517*x1*x2
        dx3 = 0.1226*x1*x2 - 0.1184*x3 + 0.0956*x0
