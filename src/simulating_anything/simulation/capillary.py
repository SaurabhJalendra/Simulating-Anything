...
class CapillarySimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0457*x1 - 0.0876*x0 + 0.0675*np.sin(0.0468*self._t)
        dx1 = -0.1111*x0*x1 + 0.0467*x2 - 0.0347*x1
        dx2 = 0.0487*x0 - 0.0760*x2 + 0.0594*x3 - 0.0346*x1*x2
        dx3 = 0.0542*x1*x2 - 0.0367*x3 + 0.0674*x0
