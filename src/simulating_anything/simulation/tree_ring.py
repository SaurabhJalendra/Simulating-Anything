...
class TreeRingSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0207*x1 - 0.0750*x0 + 0.0836*np.sin(0.0244*self._t)
        dx1 = -0.0908*x0*x1 + 0.0604*x2 - 0.0442*x1
        dx2 = 0.0216*x0 - 0.0422*x2 + 0.0672*x3 - 0.0924*x1*x2
        dx3 = 0.0739*x1*x2 - 0.0643*x3 + 0.0389*x0
