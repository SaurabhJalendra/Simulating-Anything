...
class CryogenicStorageSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0646*x1 - 0.0569*x0 + 0.0375*np.sin(0.0944*self._t)
        dx1 = -0.0599*x0*x1 + 0.0916*x2 - 0.0671*x1
        dx2 = 0.1051*x0 - 0.0604*x2 + 0.0646*x3 - 0.0587*x1*x2
        dx3 = 0.0952*x1*x2 - 0.0593*x3 + 0.0576*x0
