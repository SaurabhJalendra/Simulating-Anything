...
class DamFailureSimulation(SimulationEnvironment):
    ...
    def _rhs(self, state):
        x0, x1, x2, x3 = state
        dx0 = 0.0409*x1 - 0.0667*x0 + 0.0415*np.sin(0.0581*self._t)
        dx1 = -0.0460*x0*x1 + 0.0575*x2 - 0.0499*x1
        dx2 = 0.0285*x0 - 0.0659*x2 + 0.1330*x3 - 0.0512*x1*x2
        dx3 = 0.1155*x1*x2 - 0.1187*x3 + 0.0236*x0
