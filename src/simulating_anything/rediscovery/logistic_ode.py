"""Continuous logistic (Verhulst) ODE rediscovery.

Targets:
- Analytical solution: N(t) = K / (1 + (K/N_0 - 1)*exp(-r*t)) via trajectory match
- Inflection point: N_inflection = K / 2 via PySR
- Maximum growth rate: dN/dt_max = r * K / 4 via PySR
...
def generate_growth_data(
    ...
    Sweeps r and K, records inflection point and max growth rate for each
    ...
    Returns:
        Dict with arrays: 'r', 'K', 'inflection_N', 'max_dNdt', 'final_N'.
    """
    n_r = int(np.sqrt(n_samples))
    n_K = n_samples // n_r

    r_values = np.linspace(0.1, 3.0, n_r)
...
def generate_inflection_data(
    ...
    The inflection point of the logistic model is at N = K/2, so we sweep K
    and record both the theoretical and measured inflection points.

    Args:
        n_K: number of K values.
        r: intrinsic growth rate (fixed).
        N_0: initial population.
        dt: timestep.
        n_steps: steps per run.

    Returns:
        Dict with 'K', 'inflection_N_theory', 'inflection_N_measured',
        'max_dNdt_theory', 'max_dNdt_measured'.
    """
    ...
    for K_val in K_values:
        ...
        inflection_theory.append(K_val / 2.0)
        inflection_measured.append(N_at_max_rate)
        max_dNdt_theory.append(r * K_val / 4.0)
...
def run_logistic_ode_rediscovery(
    output_dir: str | Path = "output/rediscovery/logistic_ode",
    n_iterations: int = 40,
) -> dict:
    """Run continuous logistic ODE rediscovery pipeline.

    Discovers:
    1. Analytical solution match: N(t) = K / (1 + (K/N_0 - 1)*exp(-r*t))
    2. Inflection point: N = K/2 via PySR
    3. Maximum growth rate: dN/dt_max = r*K/4 via PySR
    ...
    results: dict = {
        "domain": "logistic_ode",
        "targets": {
            "analytical": "N(t) = K / (1 + (K/N_0 - 1)*exp(-r*t))",
            "inflection": "N_inflection = K / 2",
            "max_growth_rate": "dN/dt_max = r * K / 4",
            "ode": "dN/dt = r * N * (1 - N/K)",
        },
    }
    ...
    infl_data = generate_inflection_data(
        n_K=30, r=1.0, N_0=1.0, dt=0.05, n_steps=2000,
    )
    ...
    logger.info("Part 3: Max growth rate = r*K/4...")
    ...
    ode_data = generate_ode_data(r=1.0, K=100.0, N_0=1.0, n_steps=5000, dt=0.01)
    ...
    logger.info("Part 5: Fixed point verification...")
    fp_sim = LogisticODESimulation(
        _make_config(r=1.0, K=100.0, N_0=1.0)
    )
    fp_sim.reset()
    fixed_pts = fp_sim.find_fixed_points()
    results["fixed_points"] = [
        {"N": fp["N"], "stability": fp["stability"], "eigenvalue": fp["eigenvalue"]}
        for fp in fixed_pts
    ]
    for fp in fixed_pts:
        logger.info(
            f"  N*={fp['N']:.1f}: {fp['stability']} (eig={fp['eigenvalue']:.4f})"
        )
