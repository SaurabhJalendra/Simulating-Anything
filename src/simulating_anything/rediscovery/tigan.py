"""Tigan (T-system) rediscovery.

Targets:
- SINDy recovery of Tigan ODEs: x'=a*(y-x), y'=(c-a)*x-a*x*z, z'=-b*z+x*y
...
def generate_ode_data(
    ...
    """Generate a single Tigan trajectory for SINDy ODE recovery.

    Uses standard chaotic parameters by default.
    """
    config = SimulationConfig(
        domain=_TIGAN_DOMAIN,
        dt=dt,
        n_steps=n_steps,
        parameters={
            "a": a,
...
def generate_chaos_transition_data(
    ...
    For the Tigan system with a=2.1, b=0.6, chaos emerges as c increases.
    """
    c_values = np.linspace(5.0, 50.0, n_c)
    lyapunov_exps = []
    attractor_types = []
    max_amplitudes = []
...
def generate_lyapunov_vs_c_data(
    ...
    Focuses on c in [15, 45] to capture the chaotic regime transitions.
    """
    c_values = np.linspace(15.0, 45.0, n_c)
    lyapunov_exps = []
...
def run_tigan_rediscovery(
    output_dir: str | Path = "output/rediscovery/tigan",
    n_iterations: int = 40,
) -> dict:
    """Run the full Tigan (T-system) rediscovery.
    ...
    results = {
        ...
        "targets": {
            "ode_x": "dx/dt = a*(y - x)",
            "ode_y": "dy/dt = (c - a)*x - a*x*z",
            "ode_z": "dz/dt = -b*z + x*y",
            "chaos_regime": "a=2.1, b=0.6, c=30.0 (standard chaotic)",
            "divergence": "-(a + b)",
        },
    }

    # --- Part 1: SINDy ODE recovery ---
    logger.info("Part 1: Generating Tigan trajectory for SINDy...")
    ...
    fine_data = generate_lyapunov_vs_c_data(
        n_c=30, n_steps=30000, dt=0.005
    )
    ...
    sim_classic = TiganSimulation(config_classic)
    sim_classic.reset()
    for _ in range(10000):
        sim_classic.step()
    ...
    logger.info(f"  Classic Tigan Lyapunov: {lam_classic:.4f}")

    # --- Part 5: Fixed points ---
    sim_fp = TiganSimulation(config_classic)
    sim_fp.reset()
    ...
    div_expected = -(2.1 + 0.6)
    results["divergence"] = {
        "computed": float(div_computed),
        "expected": float(div_expected),
        "match": bool(np.isclose(div_computed, div_expected)),
    }
    logger.info(
        f"  Divergence: {div_computed:.4f} "
