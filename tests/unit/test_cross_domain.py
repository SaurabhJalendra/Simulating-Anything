...
class TestDomainSignatures:
    def test_build_signatures(self):
        ...
        for domain in ["duffing", "schwarzschild", "quantum_oscillator",
                       "boltzmann_gas", "spring_mass_chain"]:
            assert domain in names
...
class TestRunAnalysis:
    ...
    def test_similarity_matrix_diagonal(self, tmp_path):
        ...
        matrix = np.array(results["similarity_matrix"]["matrix"])
        n = matrix.shape[0]
        np.testing.assert_array_equal(np.diag(matrix), np.ones(n))
